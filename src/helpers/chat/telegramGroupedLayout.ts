...
function accumulate(arr: number[], initialValue: number): number {
...
function normalizeSizes(input: Size[]): Size[] {
  return input.map((s) => {
...
// Telegram-style grouped media layout (ported from Telegram Desktop grouped_layout.cpp logic).
export class Layouter {
  ...
  constructor(
    ...
    this.ratios = Layouter.countRatios(normalized);
    this.proportions = Layouter.countProportions(this.ratios);
    this.averageRatio = accumulate(this.ratios, 1) / Math.max(1, this.count);
  ...
  private layoutTwo(): GroupMediaLayout[] {
    if (this.proportions === "ww" && this.averageRatio > 1.4 * this.maxSizeRatio && this.ratios[1] - this.ratios[0] < 0.2) {
      return this.layoutTwoTopBottom();
    }
    if (this.proportions === "ww" || this.proportions === "qq") {
  ...
  private layoutOne(): GroupMediaLayout[] {
    ...
    const height = (width / this.ratios[0]) * 1;
    return [
      {
  ...
  private layoutThreeLeftAndOther(): GroupMediaLayout[] {
    ...
    const thirdHeight = Math.round(
      Math.min((this.maxHeight - this.spacing) / 2, (this.ratios[1] * (this.maxWidth - this.spacing)) / (this.ratios[2] + this.ratios[1]))
    );
    const secondHeight = firstHeight - thirdHeight - this.spacing;
    const rightWidth = Math.max(
      this.minWidth,
      Math.round(
        Math.min((this.maxWidth - this.spacing) / 2, Math.min(thirdHeight * this.ratios[2], secondHeight * this.ratios[1]))
      )
    );
  ...
  private layoutFourTopAndOther(): GroupMediaLayout[] {
    ...
    const w2 = Math.round(
      Math.max(Math.max(this.minWidth, (this.maxWidth - 2 * this.spacing) * 0.33), h * this.ratios[3])
    );
  ...
  private static countRatios(sizes: Size[]): number[] {
    return sizes.map((size) => size.w / size.h);
  }

  private static countProportions(ratios: number[]): string {
...
class ComplexLayouter {
  ...
  private static cropRatios(ratios: number[], averageRatio: number): number[] {
    const kMaxRatio = 2.75;
    const kMinRatio = 0.6667;
    return ratios.map((ratio) => {
      return averageRatio > 1.1 ? clamp(ratio, 1, kMaxRatio) : clamp(ratio, kMinRatio, 1);
    });
  }
  ...
  public layout(): GroupMediaLayout[] {
    ...
    for (const attempt of attempts) {
      ...
      const totalHeight = accumulate(heights, 0) + this.spacing * (lineCount - 1);
    ...
    const picked = optimalAttempt ?? attempts[0];
    if (!picked) return result;
    const optimalCounts = picked.lineCounts;
    const optimalHeights = picked.heights;
...
export function layoutTelegramAlbum(
  items: Size[],
  opts: {
    maxWidth: number;
    minWidth: number;
...
): { width: number; height: number; layout: GroupMediaLayout[] } {
  const maxWidth = Math.max(80, Math.round(Number(opts.maxWidth) || 0));
  const minWidth = Math.max(60, Math.round(Number(opts.minWidth) || 0));
  const spacing = Math.max(0, Math.round(Number(opts.spacing) || 0));
  const maxHeight =
    typeof opts.maxHeight === "number" && Number.isFinite(opts.maxHeight) && opts.maxHeight > 0 ? opts.maxHeight : undefined;
  const layouter =
    maxHeight !== undefined ? new Layouter(items, maxWidth, minWidth, spacing, maxHeight) : new Layouter(items, maxWidth, minWidth, spacing);
  const layout = layouter.layout();

  let width = 0;
  let height = 0;
  for (const item of layout) {
    width = Math.max(width, item.geometry.x + item.geometry.width);
    height = Math.max(height, item.geometry.y + item.geometry.height);
  }
