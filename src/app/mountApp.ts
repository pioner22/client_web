...
export function mountApp(root: HTMLElement) {
  ...
  layout.chat.addEventListener("click", (e) => {
    ...
    if (jumpBtn) {
      e.preventDefault();
      smoothScrollChatHostTo(Math.max(0, layout.chatHost.scrollHeight - layout.chatHost.clientHeight));
      const key = String(layout.chatHost.getAttribute("data-chat-key") || "");
  ...
  function smoothScrollChatHostTo(targetTop: number) {
    const host = layout.chatHost;
  ...
  function centerChatRow(row: HTMLElement) {
    const host = layout.chatHost;
    const hostRect = host.getBoundingClientRect();
