...
test("mobile app frame: contact list owns the full fixed mobile frame without a footer row slide", async () => {
  ...
  assert.match(css, /\.sidebar-mobile-bottom\s*\{[\s\S]*?align-items:\s*stretch;/);
  assert.match(css, /\.sidebar-tabs\.sidebar-tabs-mobile\.sidebar-tabs-bottom-nav\s*\{[\s\S]*?height:\s*100%;[\s\S]*?border-radius:\s*22px;/);
  ...
  assert.match(css, /\.sidebar-tabs\.sidebar-tabs-mobile\.sidebar-tabs-bottom-nav\s+\.sidebar-tab\s*\{[\s\S]*?height:\s*100%;[\s\S]*?font-size:\s*0;[\s\S]*?overflow:\s*hidden;/);
  ...
  assert.match(
    css,
    /html\.is-ios:not\(\.kbd-open\):not\(\.has-auth-pages\)\s+\.sidebar\s*\{[\s\S]*?--mobile-bottom-nav-fill:\s*var\(--app-logged-bottom-fill\);/
  );
  assert.match(
    css,
    /html\.is-ios:not\(\.kbd-open\):not\(\.has-auth-pages\)\s+\.sidebar-body\s*\{[\s\S]*?padding-bottom:\s*calc\(var\(--mobile-bottom-nav-h\)\s*\+\s*var\(--sp-2\)\);/
  );
