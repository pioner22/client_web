...
test("modal flow polish: CSS and source guards present", async () => {
  ...
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*html\.has-auth-pages,\s*body\.has-auth-pages\s*{[^}]*--auth-safe-bottom:\s*max\(10px,\s*var\(--app-bottom-live-pad\)\);[^}]*--auth-viewport-min:\s*var\(--app-vh,\s*100dvh\);[^}]*--app-frame-bg:\s*var\(--auth-mobile-screen-bg\);[^}]*background:\s*var\(--auth-mobile-screen-bg\);[^}]*overflow-y:\s*hidden;/);
