...
test("modal flow polish: CSS and source guards present", async () => {
  ...
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*#auth-pages\.auth-entry-page\s+>\s+\.auth-entry-scroll\s*{[^}]*align-items:\s*start;[^}]*justify-items:\s*center;[^}]*padding:\s*calc\(clamp\(42px,\s*8vh,\s*72px\) \+ env\(safe-area-inset-top\)\) 14px calc\(14px \+ env\(safe-area-inset-bottom\)\);/);
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*\.auth-entry-layout\s*{[^}]*height:\s*auto;[^}]*max-height:\s*calc\(var\(--app-vh,\s*100dvh\) - 54px\);[^}]*max-width:\s*420px;/);
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*\.auth-entry-panel\s*{[^}]*height:\s*auto;/);
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*\.auth-entry-panel\s*{[^}]*border:\s*1px solid var\(--auth-mobile-line\);[^}]*border-radius:\s*22px;[^}]*background:\s*linear-gradient\(180deg,\s*var\(--auth-mobile-panel\) 0%,\s*#f9fbff 100%\);/);
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*\.auth-panel-top,\s*\.modal-auth\s+\.auth-panel-top\s*{[^}]*border-radius:\s*18px;[^}]*background:\s*linear-gradient\(180deg,\s*var\(--auth-mobile-panel-top\) 0%,\s*#f8fbff 100%\);/);
