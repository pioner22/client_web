...
test("modal flow polish: CSS and source guards present", async () => {
  ...
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*\.overlay\.overlay-auth:not\(\.hidden\)\s*>\s*\.modal\s*{[^}]*width:\s*100%;[^}]*height:\s*100dvh;[^}]*border:\s*0;[^}]*background:\s*transparent;/);
  ...
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*\.auth-entry-layout\s*{[^}]*height:\s*auto;[^}]*max-height:\s*none;[^}]*min-height:\s*100dvh;[^}]*max-width:\s*none;/);
