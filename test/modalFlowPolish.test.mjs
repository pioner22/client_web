...
test("modal flow polish: CSS and source guards present", async () => {
  ...
  assert.match(css, /@media\s*\(max-width:\s*860px\)\s*{[\s\S]*#auth-pages\.auth-entry-page\s+>\s+\.auth-entry-scroll\s*{[^}]*display:\s*flex;[^}]*flex-direction:\s*column;[^}]*height:\s*100%;[^}]*align-items:\s*center;[^}]*justify-content:\s*stretch;[^}]*padding:\s*0 18px var\(--auth-scroll-bottom-space\);[^}]*overflow-y:\s*auto;[^}]*scroll-padding-bottom:\s*var\(--auth-scroll-bottom-space\);/);
