...
test("context menu/pin remediation source guards: modern sheet, compact message menu, and direct pin toggle", async () => {
  ...
  assert.match(modalCss, /\.ctx-menu\.ctx-menu-message-action-list\s*\{[\s\S]*width:\s*min\(286px,\s*calc\(100vw - 24px\)\)/);
  assert.match(modalCss, /\.ctx-menu\.ctx-menu-message-action-list\s*\{[\s\S]*background:\s*transparent;/);
  assert.match(modalCss, /\.ctx-menu\.ctx-menu-message-action-list\s+\.ctx-list\s*\{[\s\S]*width:\s*min\(var\(--ctx-action-list-w,\s*286px\),\s*calc\(100vw - 32px\)\)/);
  ...
  assert.match(skinCss, /html\[data-skin="yagodka-modern"\]\s+\.ctx-menu\.ctx-menu-message-action-list\s*\{[\s\S]*width:\s*min\(286px,\s*calc\(100vw - 24px\)\)/);
  assert.match(skinCss, /html\[data-skin="yagodka-modern"\]\s+\.ctx-menu\.ctx-menu-message-action-list\s+\.ctx-list\s*\{[\s\S]*border-radius:\s*22px;/);
